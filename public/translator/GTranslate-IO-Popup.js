...
I - DECRYPTER L'ORIGINAL SUR: https://deobfuscate.io/
II - A - REMPLACER:
******************************************************************
...
PAR:
******************************************************************
var url_structure = gt.url_structure || "sub_directory";
******************************************************************

II - B - REMPLACER EGALEMENT DANS TOUT LE CODE CI-DESSOUS TOUS LES:
******************************************************************
url_structure == "none"
******************************************************************
...
III - MODIFIER "document.querySelectorAll(u_class + " a[data-gt-lang]").forEach(function (e) { ... });" COMME SUIT:
**************************************************************************************************************************************************************************************************
document.querySelectorAll(u_class + " a[data-gt-lang]").forEach(function (e) {
  e.addEventListener("click", function (evt) {
	...
	var preferred_language = e.getAttribute("data-gt-lang");
	var newUrl = window.location.protocol + "//" + window.location.host + window.location.pathname + "?lang=" + preferred_language;
	window.history.pushState({ path: newUrl }, "", newUrl);
  });
});
************************************************************************************************************************************************************************************

IV - REMPLACER LA LIGNE SUIVANTE:
************************************************************************************************************************************************************************************
...
PAR:
************************************************************************************************************************************************************************************
href = lang == default_language && location.protocol + "//" + location.hostname + gt_request_uri || location.protocol + "//" + location.hostname + "/?lang=" + lang;
************************************************************************************************************************************************************************************

V - MODIFIER EGALEMENT "if (current_lang === default_language && preferred_language !== default_language && languages.includes(preferred_language)) { ... }" COMME SUIT:
************************************************************************************************************************************************************************************
if (current_lang === default_language && preferred_language !== default_language && languages.includes(preferred_language)) {
  if (url_structure === "sub_directory") {
	...
	document.querySelectorAll(u_class + ' a[data-gt-lang="' + preferred_language + '"]').forEach(function (e) {
	  // Change the URL without page reload
	  var newUrl = e.href + "?lang=" + preferred_language;
	  window.history.pushState({ path: newUrl }, "", newUrl);
	});
  }
}
...
VI - MODIFICATION EN DESSOUS DE LA VARIABLE "var accept_language = (navigator.language || navigator.userLanguage).toLowerCase();" COMME SUIT:
************************************************************************************************************************************************************************************
if (detect_browser_language && window.sessionStorage && window.navigator && sessionStorage.getItem("gt_autoswitch") == null && !/bot|spider|slurp|facebook/i.test(navigator.userAgent)) {
    var accept_language = (navigator.language || navigator.userLanguage).toLowerCase();
	// AJOUT DE CE QUI SUIT POUR DEFINIR LA VARIABLE "langValue":
	// On récupère dans la variable "currentURL", l'URL actuel dans la barre de navigation:
	var currentURL = window.location.href;
	var langValue = null; // Declare the variable outside the loop
	...
	if (urlParts.length > 1) {
		...
		for (var i = 0; i < parameters.length; i++) {
			...
			if (paramParts[0] === "lang") {
				// Ici on extrait la valeur du paramètre "lang" de l'URL courant (fr, da, es, de, ca, ...) dans la variable "langValue":
				langValue = paramParts[1]; // Assign the value inside the loop
				break;
			}
		}
	}
    ...
    switch (accept_language) {
	  ...
	  ...
      default:
        var preferred_language = (langValue !== null) ? langValue : accept_language.substr(0, 2);				// MODIFICATION ICI PAR LA CONDITION TERNAIRE AVEC "langValue"
        break;
    }
	...
	...
...
<script src="https://worklx.tech/translator/GTranslate-IO-Popup.js" defer></script>
*******************************************************************************************
...
(function () {
  ...
  function get_flag_src(lang) {
    if (!alt_flags[lang]) return flags_location + lang + flag_ext; else if (alt_flags[lang] == "usa") return flags_location + "en-us" + flag_ext; else if (alt_flags[lang] == "canada") return flags_location + "en-ca" + flag_ext; else if (alt_flags[lang] == "brazil") return flags_location + "pt-br" + flag_ext; else if (alt_flags[lang] == "mexico") return flags_location + "es-mx" + flag_ext; else if (alt_flags[lang] == "argentina") return flags_location + "es-ar" + flag_ext; else if (alt_flags[lang] == "colombia") return flags_location + "es-co" + flag_ext; else if (alt_flags[lang] == "quebec") return flags_location + "fr-qc" + flag_ext; else return alt_flags[lang];
  }
  function get_lang_href(lang) {
    var href = "#";
    if (url_structure == "sub_directory") {
      var gt_request_uri = (document.currentScript.getAttribute("data-gt-orig-url") || (location.pathname.startsWith("/" + current_lang + "/") && "/" + location.pathname.split("/").slice(2).join("/") || location.pathname)) + location.search + location.hash;
      // href = lang == default_language && location.protocol + "//" + location.hostname + gt_request_uri || location.protocol + "//" + location.hostname + "/?lang=" + lang + gt_request_uri;
      href = lang == default_language && location.protocol + "//" + location.hostname + gt_request_uri || location.protocol + "//" + location.hostname + "/?lang=" + lang;
    } else if (url_structure == "sub_domain") {
      var gt_request_uri = (document.currentScript.getAttribute("data-gt-orig-url") || location.pathname) + location.search + location.hash;
      var domain = document.currentScript.getAttribute("data-gt-orig-domain") || location.hostname;
      if (typeof custom_domains == "object" && custom_domains[lang]) href = lang == default_language && location.protocol + "//" + domain + gt_request_uri || location.protocol + "//" + custom_domains[lang] + gt_request_uri; else href = lang == default_language && location.protocol + "//" + domain + gt_request_uri || location.protocol + "//" + lang + "." + domain.replace(/^www\./, "") + gt_request_uri;
    }
    return href;
  }
  widget_css += "a.glink{text-decoration:none}a.glink.gt-current-lang{font-weight:bold}";
  var font_size = 10, margin_right = 3;
  if (flag_size == 24) font_size = 15, margin_right = 5; else if (flag_size == 32) font_size = 20, margin_right = 7; else if (flag_size == 48) font_size = 24, margin_right = 10;
  ...
  if (horizontal_position != "inline") widget_code = '<div class="gt_switcher_wrapper" style="position:fixed;' + vertical_position + ":15px;" + horizontal_position + ':15px;z-index:999999;">' + widget_code + "</div>";
  ...
  document.querySelectorAll(u_class + " a.gt_switcher-popup").forEach(function (e) {
    e.addEventListener("click", function (evt) {
      evt.preventDefault();
      evt.stopPropagation();
      if (gt_popup_open) gt_hide_popup(); else gt_show_popup(e);
    });
  ...
  if (url_structure == "sub_directory") {
    ...
    if (current_lang != default_language) load_tlib(); else document.querySelectorAll(u_class).forEach(function (e) {
  ...
  if (detect_browser_language && window.sessionStorage && window.navigator && sessionStorage.getItem("gt_autoswitch") == null && !/bot|spider|slurp|facebook/i.test(navigator.userAgent)) {
    ...
    switch (accept_language) {
      ...
      case "he":
        ...
        break;
      default:
        var preferred_language = (langValue !== null) ? langValue : accept_language.substr(0, 2);
        break;
    }
    ...
    /* if (current_lang == default_language && preferred_language != default_language && languages.includes(preferred_language)) {
      ...
      } else document.querySelectorAll(u_class + ' a[data-gt-lang="' + preferred_language + '"]').forEach(function (e) {
